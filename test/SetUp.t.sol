   ...
   // MockIchiVault ichi_USDC_ICHI_Vault;
    ...
    function test_openPosition() public {
        ...
        uint256 positionId = bank.execute(0, address(spell),abi.encodeWithSignature("openPosition(uint256,address,address,uint256,uint256)",
                                                    ...
                                                    borrowAmount ));
        vm.stopPrank();
       uint256 AliceBalance = werc20.balanceOfERC20(address(ichi_USDC_ICHI_Vault), address(bank));
        console.log("alice `positionId` is ", positionId);
        console.log("bank own ichi_USDC_ICHI_Vault :", AliceBalance);

    }

    function test_SoftVault_Deposit() public {
        uint256 amount =300e6;
        uint256 time =300 weeks;

        bank.setAllowContractCalls(true);
         console.log("Alice Balance Of USDC Before `Deposit()`", IERC20(USDC).balanceOf(alice));

        vm.startPrank(alice);
        IERC20(USDC).approve(address(usdcSoftVault), type(uint256).max);
        usdcSoftVault.deposit(amount);
        vm.warp(time);

       amount = IERC20Upgradeable(usdcSoftVault).balanceOf(alice);

        console.log("Alice Balance Of USDC After `Deposit()`", IERC20(USDC).balanceOf(alice));
        usdcSoftVault.withdraw(amount);

        console.log("Alice Balance Of USDC After `withdraw()`", IERC20(USDC).balanceOf(alice));
        vm.stopPrank();
    ...
    function test_Attack02() public {//!@audit-issue -H- Users could leave some of ICHI Vault LP on SPELL 
        uint256 strategyId = 0;
        address collToken = ICHI;
        address borrowToken = USDC;
        ...
        console.log("==========================================================");
        console.log("SPELL Balance Of ICHI_Vault_LP  After `execute(openPosition)`", IERC20(ichi_USDC_ICHI_Vault).balanceOf(address(spell)));
        console.log("SPELL Balance Of USDC  After `execute(openPosition)`", IERC20(USDC).balanceOf(address(spell)));
        console.log("SPELL Balance Of ICHI  After `execute(openPosition)`", IERC20(ICHI).balanceOf(address(spell)));
        console.log("==========================================================");
        console.log("Alice Balance Of ICHI_Vault_LP  After `execute(openPosition)`", IERC20(ichi_USDC_ICHI_Vault).balanceOf(alice));
        console.log("Alice Balance Of USDC  After `execute(openPosition)`", IERC20(USDC).balanceOf(alice));
        console.log("Alice Balance Of ICHI  After `execute(openPosition)`", IERC20(ICHI).balanceOf(alice));
        console.log("==========================================================");

        uint256 lpTakeAmt = type(uint256).max;
        uint256 amountRepay = 2500e6;//2500 ==> only 50%
        uint256 amountLpWithdraw = 2500e6;//2500 ==> only 50%
        uint256 amountShareWithdraw = 0;
    ...
    function test_Attack04() public {//!@audit-issue -H-
        uint256 strategyId = 0;
        address collToken = ICHI;
        address borrowToken = USDC;
        uint256 collAmount = 1050e18;//$5 * 1050 = $5250
        uint256 borrowAmount =15000e6;//$1 * 15000 = $15000
        uint256 time =300 weeks;
        ...
        //pxs[3] = 24000;
        mockOracle.setPrice(tokens, pxs);
        bool isLiquidatable = bank.isLiquidatable(positionId);
        console.log("is Alice Position Liquidatable`", isLiquidatable);
        uint256 price = mockOracle.getPrice(ICHI);
        console.log("ICHI price IS :", price);
