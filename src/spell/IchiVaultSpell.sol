...
contract IchiVaultSpell is BasicSpell, IUniswapV3SwapCallback {
     ...
     */                                                                 //!@audit-issue -M- Adversary users can keep invoking this function with `amount == 0`
                                                                        //! this will lead to DOS Atack because the modifier `inExec()`+ creat multipul accont
    function increasePosition(address token, uint256 amount) external {//!@audit-issue -H- users can open a new position at the same time invok this function, this will set `pos.underlyingToken` to any token that on `whitelistedTokens[]`  
        // 1. Get user input amounts                                     //! than user will invoke `openPosition()` with an other token
        doLend(token, amount);
    }
     ...
     * @param collAmount Amount of Isolated collateral
     */
    function reducePosition(//!@audit-issue -H01.1- any user can invoke this function even with a new opend position with no money on it 
        uint256 strategyId,
        address collToken,
        uint256 collAmount//! and just set this amount to any number and all the tokens will lock there on **BlueBerryBank.sol**
    ) external {
    ...
    function withdrawInternal(
        ...
        doWithdraw(collToken, amountShareWithdraw);//!@audit-issue -In case `collToken == borrowToken` and the user by mistake set `amountRepay` to a the Max or to a big value
                                                     //!It will repay some 
        // 6. Repay
