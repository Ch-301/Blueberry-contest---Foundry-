...
contract BlueBerryBank is OwnableUpgradeable, ERC1155NaiveReceiver, IBank {
    ...
    {
        Position storage pos = positions[positionId];
        uint256 pv = getPositionValue(positionId);//1155 ==> ICHI Vault LP ==> USDC (borrowAmount)//!100 USD
        uint256 ov = getDebtValue(positionId);//USD value total debt //!24000 USD
        uint256 cv = oracle.getUnderlyingValue(//!9950 USD
            pos.underlyingToken,
            pos.underlyingAmount
        );
        console.log("=======================");
         console.log("getPositionValue Is:",pv);
         console.log("getDebtValue Is:",ov);
         console.log("getUnderlyingValue Is:",cv);
         console.log("=======================");
    ...
    {
        Position storage pos = positions[positionId];
        uint256 risk = getPositionRisk(positionId);//24020

       uint256 xLiqThreshold = oracle.getLiqThreshold(pos.underlyingToken);
       /* console.log("risk Is:",risk);
        console.log("xLiqThreshold Is:",xLiqThreshold);*/

        liquidatable = risk >= xLiqThreshold;//8000 >= 9000
    }
    ...
    function execute(
        ...
        (bool ok, bytes memory returndata) = SPELL.call{value: msg.value}(data);//!@audit -M- this function will keep reverting because the SPELL Can only receive ETH from WETH contract 
        if (!ok) {
        ...
        poke(token)
    {
        ...
        wAmount = wAmount > pos.underlyingAmount
            ? pos.underlyingAmount //!@audit-issue (Done) -H01.2- the reast of the tokens will locked here
            : wAmount;
    ...
    function repayInternal(
        ...
        uint256 totalShare = bank.totalShare;
        uint256 totalDebt = bank.totalDebt;
        uint256 oldShare = pos.debtShareOf[token];//!==>0
        uint256 oldDebt = (oldShare * totalDebt).divCeil(totalShare);//!==>0
